...
/**
 * Store a single hit directly into the country geolocation aggregation table.
 */
function bbpa_store_geo_aggregate_hit(array $hit): void
{
    ...
    $row = [
        ...
        'hits' => 1,
        'visits' => $page_path !== '' && bbpa_is_entry_hit($page_path, $referrer_domain) ? 1 : 0,
    ];
    $row = apply_filters('bbpa_geo_aggregate_row', $row, $geo, $hit);
    bbpa_upsert_geo_rows([$row]);
}
...
/**
 * Upsert country-level geolocation aggregate rows.
 */
function bbpa_upsert_geo_rows(array $rows): int
{
    if ($rows === []) return 0;
    $premium_result = apply_filters('bbpa_geo_rows_upsert_result', null, $rows);
    if (is_int($premium_result)) return $premium_result;
    ...
    foreach ($rows as $row) {
        $placeholders[] = '(%s, %s, %d, %d)';
        $values[] = $row['date_bucket'];
        $values[] = bbpa_normalize_country_code($row['country_code'] ?? '');
        $values[] = (int) ($row['hits'] ?? 0);
        $values[] = (int) ($row['visits'] ?? 0);
    }
    $sql = "INSERT INTO {$table} (date_bucket, country_code, hits, visits) VALUES "
        . implode(', ', $placeholders)
        . ' ON DUPLICATE KEY UPDATE hits = hits + VALUES(hits), visits = visits + VALUES(visits)';
