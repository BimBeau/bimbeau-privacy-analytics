...
if (!function_exists('bbpa_get_package_activation_conflict_message')) {
     ...
     */
    function bbpa_get_package_activation_conflict_message(string $plugin_file): string
    {
        ...
        return sprintf(
            /* translators: 1: active package name, 2: package whose activation was refused. */
            __('%1$s is already active. Deactivate it before activating %2$s.', 'bimbeau-privacy-analytics'),
...
if (!function_exists('bbpa_prevent_parallel_package_activation')) {
    ...
    function bbpa_prevent_parallel_package_activation(string $plugin_file, bool $network_wide = false): void
    {
        ...
        $message = bbpa_get_package_activation_conflict_message($plugin_file);
        bbpa_store_activation_notice($message, 'error');

        $plugins_url = $network_wide && function_exists('network_admin_url')
            ? network_admin_url('plugins.php')
            : admin_url('plugins.php');

        // Replace activate_plugin()'s prepared error redirect before ending the activation request.
...
if (!function_exists('bbpa_resolve_package_activation_conflict')) {
    ...
    function bbpa_resolve_package_activation_conflict(string $plugin_file): void
    {
        ...
        bbpa_store_activation_notice(
            sprintf(
                /* translators: 1: package loaded first, 2: other package also marked active. */
                __('Both %1$s and %2$s are marked active. Only one package can be active at a time; deactivate one of them.', 'bimbeau-privacy-analytics'),
                bbpa_get_package_label($current_basename),
                bbpa_get_package_label($conflicting_basename)
            ),
            'warning'
