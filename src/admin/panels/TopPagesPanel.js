...
const TopPagesPanel = ( { rangeSelection } ) => {
	...
	const canUsePageDetails = isPro;
	const initialPageDetailsSelection = useMemo(
		() =>
			canUsePageDetails
				? getInitialPageDetailsSelection()
				: { selectedPage: null, selectedSource: 'top-pages' },
		[ canUsePageDetails ]
	);
	...
	useEffect( () => {
		...
		const onPopState = () => {
			if ( ! canUsePageDetails ) {
				setSelectedPage( null );
				setSelectedSource( 'top-pages' );
				return;
			}
	...
	useEffect( () => {
		...
		const params = new URLSearchParams( window.location.search );
		if (
			! params.has( 'bbpa_detail_page' ) &&
			! params.has( 'bbpa_detail_source' )
		) {
			return;
		}

		params.delete( 'bbpa_detail_page' );
		params.delete( 'bbpa_detail_source' );
	...
	const pagesTabs = [
		...
		{
			name: 'not-found',
			title: __( 'Pages not found', 'bimbeau-privacy-analytics' ),
		},
	];

	if ( selectedPage && canUsePageDetails ) {
		return (
			<PageDetailsPanel
				range={ range }
	...
	return (
		<div className="bbpa-report-panel">
			...
			<BpaCard
				className="bbpa-pages-listings-card"
				title={ __( 'Pages', 'bimbeau-privacy-analytics' ) }
			>
				<TabPanel
					className="bbpa-pages-tabs"
					...
					{ ( tab ) => {
						...
						if ( tab.name === 'not-found' ) {
							return (
								<NotFoundPanel
									range={ range }
									onOpenDetails={ openDetails( '404s' ) }
								/>
							);
						}
						return (
