...
import ProBadge, { ProBadgeText } from "../components/ProBadge";
import {
  chartBar,
  cloud,
  comment,
...
import PwaStatsAppCard from "../components/PwaStatsAppCard";
...
const SettingsPanel = () => {
  ...
  const handleEventsChange = useCallback((nextEvents) => {
    const normalizedEvents = Array.isArray(nextEvents) ? nextEvents : [];

    setFormState((prev) => {
      const previousEvents = Array.isArray(prev.free_events_config)
        ? prev.free_events_config
        : [];

      if (JSON.stringify(previousEvents) === JSON.stringify(normalizedEvents)) {
        return prev;
      }

      return {
        ...prev,
        free_events_config: normalizedEvents,
      };
    });
  }, []);
  ...
  const settingsTabs = [
    ...
    ...(isPro
      ? [
        {
          name: "analytics-app",
          title: __("App", "bimbeau-privacy-analytics"),
        },
        {
          name: "events",
              ...
              <div className="bbpa-settings-form">
                ...
                {activeTab === "general" && (
                  <div className="bbpa-general-settings">
                    ...
                    {isPro && (
                    <Card className="bbpa-general-settings__card">
                      <CardHeader className="bbpa-settings-plugin-identity-card__header">
                        <Flex gap={2} align="center">
                          <SettingsSectionTitle icon={LuBookmark}>
                            {__("Interface branding", "bimbeau-privacy-analytics")}
                          </SettingsSectionTitle>
                          <ProBadge />
                        </Flex>
                      </CardHeader>
                        ...
                        <fieldset disabled={!isPro} className="bbpa-pro-feature-fieldset">
                          {!isPro && (
                            <Notice status="warning" isDismissible={false}>
                              <ProBadgeText
                                text={__(
                                  "White-label interface controls are available in Pro.",
                                  "bimbeau-privacy-analytics",
                                )}
                              />
                            </Notice>
                          ...
                          <ToggleControl
                            label={__(
                              "Use white-label interface",
                              "bimbeau-privacy-analytics",
                            )}
                            help={__(
                              "Hide the BimBeau Privacy Analytics logo in the admin header and display the localized analytics title instead.",
                              "bimbeau-privacy-analytics",
                            )}
                            checked={Boolean(formState.white_label_enabled)}
                            onChange={(value) =>
                              setFormState((prev) => ({
                                ...prev,
                                white_label_enabled: Boolean(value),
                              }))
                            }
                          />
                        </fieldset>
                ...
                {isPro && activeTab === "analytics-app" && (
                  <PwaStatsAppCard
                    isPro={isPro}
                    pwaAppUrl={pwaAppUrl}
                    pwaNotice={pwaNotice}
                    onCopyPwaUrl={onCopyPwaUrl}
                    onOpenPwaUrl={onOpenPwaUrl}
                  />
                )}

                {isPro && activeTab === "general" && (
                  <Card className="bbpa-general-settings__card">
                    <CardHeader>
                      <Flex gap={2} align="flex-start" justify="space-between">
                        <div>
                          <SettingsSectionTitle icon={LuPanelLeft}>
                            {__("Quick stats", "bimbeau-privacy-analytics")}
                          </SettingsSectionTitle>
                          <p className="bbpa-general-settings__helper">
                            {__(
                              "Enable Quick Stats on selected post types: add a Views (30d) column in the posts list and display a stats summary metabox while editing each post.",
                              "bimbeau-privacy-analytics",
                            )}
                          </p>
                        </div>
                    ...
                    <CardBody>
                      <fieldset disabled={!isPro} className="bbpa-pro-feature-fieldset">
                        ...
                        <div className="bbpa-settings-roles__list">
                          {postTypes.length === 0 && (
                            <p>{__("No post types available.", "bimbeau-privacy-analytics")}</p>
                          )}
                          {postTypes.map((postType) => (
                            <CheckboxControl
                              key={postType.key}
                              label={postType.label}
                              checked={formState.pro_content_stats_post_types.includes(
                                postType.key,
                              )}
                              onChange={(isChecked) => {
                                setFormState((prev) => {
                                  const nextTypes = new Set(
                                    prev.pro_content_stats_post_types,
                                  );
                                  if (isChecked) {
                                    nextTypes.add(postType.key);
                                  } else {
                                    nextTypes.delete(postType.key);
                                  }

                                  return {
                                    ...prev,
                                    pro_content_stats_post_types: Array.from(nextTypes),
                                  };
                                });
                              }}
                            />
                      ...
                      {isApiLookupMode ? (
                        ...
                        <>
                          <SelectControl
                            ...
                            help={__(
                              "Controls the cron frequency for local GeoIP database updates.",
                              "bimbeau-privacy-analytics",
                            )}
                            onChange={(value) =>
                              setFormState((prev) => ({
                          ...
                          <p>{`${__("Next scheduled retry", "bimbeau-privacy-analytics")}: ${geoIpNextRun}`}</p>
