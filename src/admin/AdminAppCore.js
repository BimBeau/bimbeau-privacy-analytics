...
const AdminAppCore = ( { appContext = 'admin', hasPremiumAccess = false, HeaderBrand = null, Navigation = null, extensionNotices = null } ) => {
	...
	const restartSetupWizard = useCallback( async () => {
			if ( setupWizardResetRef.current ) {
				return setupWizardResetRef.current;
			}

			try {
				setSetupNotice( false );
				setIsSetupWizardRestarting( true );
				...
				if ( isSetupWizardMountedRef.current ) {
					setIsSetupWizardRestarting( false );
				}
			}
	}, [ logger ] );
			...
			<div className="bbpa-admin-app__auth-required">
				<p>
					{ authRequiredError?.message ||
						__(
							'Your WordPress session has expired. Reload this page.',
							'bimbeau-privacy-analytics'
						) }
				</p>
				{ authDiagnostics ? (
	...
	if ( ! isAuthRequired ) {
		panelContent = PanelComponent ? (
			<PanelComponent
				...
				onRestartSetupWizard={ restartSetupWizard }
				isSetupWizardRestarting={ isSetupWizardRestarting }
			/>
		) : (
	...
	return (
		...
		>
			<div className="bbpa-admin-app__header">
				...
				{ currentPanel !== 'settings' ? <div className="bbpa-admin-app__period-filter">
					<PeriodFilter
			...
			<div className="bbpa-admin-app__body">
				{ Navigation ? <Navigation panels={ visiblePanels } currentPanel={ currentPanel } settings={ ADMIN_CONFIG?.settings } /> : null }
