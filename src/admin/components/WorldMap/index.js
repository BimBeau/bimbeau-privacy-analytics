...
const getMapFeatureLayerNode = (svg) => {
  ...
  const candidateGroups = Array.from(svg.querySelectorAll("g")).filter(
    (group) => {
      if (
        group.getAttribute(LEGEND_LAYER_ATTRIBUTE) === LEGEND_LAYER_VALUE ||
        group.querySelector(
  ...
  const bestGroup = groupsToScore
    .map((group) => {
      ...
      const directPathCount = group.querySelectorAll(":scope > path").length;
      const hasLegendNodes = Boolean(group.querySelector("rect, text"));
...
const isMatchingPointer = (activePointerId, eventPointerId) => {
  if (Number.isFinite(activePointerId) && Number.isFinite(eventPointerId)) {
...
const findFeaturePathNode = (featureLayer, featureId) => {
  ...
  return Array.from(featureLayer.querySelectorAll("path")).find((pathNode) =>
...
export const summarizeCityMarkerDiagnostics = (cities = []) => {
  ...
  cities.forEach((city, index) => {
    ...
    } else if (Math.abs(latitude) < 0.0001 && Math.abs(longitude) < 0.0001) {
    ...
    excludedCities.push({
      ...
      hits,
      latitude: Number.isFinite(latitude) ? Number(latitude.toFixed(5)) : null,
...
const buildLegendRanges = (roundedDomainMax, bucketCount, roundingStep) => {
  ...
  for (let index = 0; index < bucketCount; index += 1) {
    ...
    const rawEnd =
      Math.ceil(((index + 1) * roundedDomainMax) / bucketCount / roundingStep) *
      roundingStep;
...
const buildCompactLegendRanges = (roundedDomainMax) =>
  buildLegendRanges(
    roundedDomainMax,
    Math.max(1, Math.min(MAX_DATA_BUCKETS, Math.ceil(roundedDomainMax / 20))),
    10,
  );

const buildMapColorBuckets = (
  maxValue,
...
const getFeatureJoinCode = (feature, joinKey) => {
  ...
  if (joinKey === GEO_JOIN_KEYS.FEATURE_ID) {
    ...
    return firstKnownCode([...isoA2CodeCandidates, ...isoA3CodeCandidates]);
  }
...
export const WorldChoropleth = ({
  ...
  const bringFeaturePathToFront = useCallback(
    (featureId, eventTarget) => {
      ...
      const reorderTarget = getReorderTargetNode(featureLayer, pathNode);
  ...
  const colorScale = useMemo(() => {
    ...
    return scaleQuantize().domain([0, domainMax]).range(countryColorRange);
  }, [countryColorRange, countryColorScale, maxDomainValue]);
  const neutralColorScale = useMemo(
    () => scaleQuantize().domain([0, 1]).range([NO_DATA_COLOR, NO_DATA_COLOR]),
    [],
  );

  const handleCountryMouseEnter = useCallback(
    (feature, event) => {
      const normalizedFeatureId = normalizeCountryCode(feature?.id || "");
      bringFeaturePathToFront(normalizedFeatureId, event?.nativeEvent?.target);
    },
...
const WorldMapTooltip = ({ data }) => (
  <div className="bbpa-world-map__tooltip">
    ...
    {data.shareLabel && (
      <div className="bbpa-world-map__tooltip-share">{data.shareLabel}</div>
    )}
  </div>
);
...
const WorldMap = ({
  ...
  const [mapThemeColor, setMapThemeColor] = useState(resolveMapThemeColor);
  ...
  const updateMapContentBounds = useCallback(() => {
    ...
    const svgRect = svg.getBoundingClientRect?.();
    const layerRect = layer.getBoundingClientRect?.();
    if (!svgRect || !layerRect || svgRect.width <= 0 || svgRect.height <= 0) {
      return;
    }
  ...
  const applyZoomAtPoint = useCallback(
    (nextScale, pointX, pointY) => {
      ...
      setViewport((previousViewport) => {
        ...
        const previousTranslateY = Number(previousViewport?.translateY) || 0;
  ...
  const handleMapDoubleClick = useCallback(
    (event) => {
      ...
      const svg = mapSvgRef.current;
      const pointer = getSvgPointFromClient(svg, event.clientX, event.clientY);
      const anchorX = pointer?.x ?? choroplethWidth / 2;
      const anchorY = pointer?.y ?? choroplethHeight / 2;

      applyZoomAtPoint(viewport.scale + VIEWPORT_ZOOM_STEP, anchorX, anchorY);
    },
    [applyZoomAtPoint, choroplethHeight, choroplethWidth, viewport.scale],
  );
  ...
  const error =
    errorOverride !== undefined ? errorOverride : endpointState.error;
  ...
  const handleMapPointerMove = useCallback(
    (event) => {
      ...
      if (
        !activeDragState ||
        !isMatchingPointer(activeDragState.pointerId, event.pointerId)
      ) {
        return;
      }

      event.preventDefault();
      const nextClientX = Number(event.clientX) || activeDragState.lastClientX;
      const nextClientY = Number(event.clientY) || activeDragState.lastClientY;
      ...
      setViewport((previousViewport) =>
        clampViewport({
          ...previousViewport,
          translateX: (Number(previousViewport?.translateX) || 0) + deltaX,
          translateY: (Number(previousViewport?.translateY) || 0) + deltaY,
        }),
      );
    },
  ...
  const handleMapPointerUp = useCallback(
    (event) => {
      ...
      if (
        dragStateRef.current &&
        isMatchingPointer(dragStateRef.current.pointerId, event.pointerId)
      ) {
        stopDrag();
      }
    },
  ...
  useEffect(() => {
    ...
    if (window.ResizeObserver) {
      const observer = new window.ResizeObserver(() => updateMapDimensions());
  ...
  const countryCenterByCode = useMemo(() => {
    ...
    const features = Array.isArray(worldGeo?.features) ? worldGeo.features : [];
    ...
    return countries
      .map((city, index) => {
        ...
        const hasZeroCoordinates =
          Math.abs(rawLatitude) < 0.0001 && Math.abs(rawLongitude) < 0.0001;
  ...
  const normalizeCountryForJoinKey = useCallback(
    (code) => {
      ...
      if (geoJoinKey === GEO_JOIN_KEYS.ISO_A2 && normalized.length === 3) {
        return isoA3ToA2.get(normalized) || "";
      }

      if (geoJoinKey === GEO_JOIN_KEYS.ISO_A3 && normalized.length === 2) {
        return isoA2ToA3.get(normalized) || "";
      }

      const byName = countryNameToJoinCode.get(normalizeCountryNameKey(code));
      if (byName) {
        return byName;
      }
  ...
  const { hitLookup, chartData, unresolvedCountryCodeCounts } = useMemo(() => {
    ...
    countries.forEach((country) => {
      ...
      if (!resolvedCode) {
        ...
        if (unresolvedCode && !isUnknownCountryCode(unresolvedCode)) {
          const currentCount = unresolvedCounts.get(unresolvedCode) || 0;
          unresolvedCounts.set(unresolvedCode, currentCount + 1);
        }
        return;
      }
  ...
  const joinDebugMetrics = useMemo(() => {
    ...
    const unresolvedSample = Array.from(unresolvedCountryCodeCounts.entries())
  ...
  useEffect(() => {
    ...
    const summary = Array.from(unresolvedCountryCodeCountsRef.current).map(
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line no-console
    console.warn(
      "[BimBeau Privacy Analytics] WorldMap country join mismatch detected",
      {
        geoJoinKey,
        totalHits,
  ...
  const maxDomainValue = useMemo(() => {
    ...
    if (isMarkerMapMode(mapMode)) {
      return Math.max(
        ...cityMarkers.map((marker) => sanitizeMapValue(marker.hits)),
        0,
      );
    }
  ...
  const colorScale = useMemo(() => {
    ...
    return scaleThreshold().domain(thresholds).range(mapColorBuckets.colors);
  }, [mapColorBuckets]);
  ...
  useEffect(() => {
    ...
    return undefined;
  }, [choroplethWidth, choroplethHeight, chartData.length, isLoading, error]);
  ...
  useEffect(() => {
    setViewport((previousViewport) => clampViewport(previousViewport));
  }, [clampViewport]);
  ...
  useEffect(() => {
    ...
    const projectedPoints = cityMarkers
      .map((marker) => defaultProjection([marker.longitude, marker.latitude]))
      .filter(
        (point) =>
          Array.isArray(point) &&
          Number.isFinite(point[0]) &&
    ...
    const spanX = Math.max(
      maxX - minX,
      width * REALTIME_MARKER_SINGLE_POINT_SPAN_RATIO,
    );
    ...
    const fittedScale = clamp(
      Math.min(
        width / Math.max(paddedSpanX, 1),
        height / Math.max(paddedSpanY, 1),
      ),
      VIEWPORT_ZOOM_MIN,
      VIEWPORT_ZOOM_MAX,
    );
  ...
  const getTooltipData = (feature) => {
    const code =
      getFeatureJoinCode(feature, geoJoinKey) || resolveFeatureId(feature);
    const flagCode =
    ...
    const hits = !isUnknown && code ? sanitizeMapValue(hitLookup.get(code)) : 0;
  ...
  const updateCityTooltipPosition = useCallback(
    (marker, event) => {
      ...
      const markerCenterX = targetRect
        ...
        : 0;
      const markerTopY = targetRect ? targetRect.top - containerRect.top : 0;
        ...
        <>
          {hasData === false &&
            ...
            shouldShowGeolocationHelperMessage && (
              <p className="bbpa-world-map__empty-message" role="status">
            ...
            >
              <WorldChoropleth
                ...
                tooltipRenderer={({ feature }) => {
                  ...
                  return (
                    <WorldMapTooltip
                      key={tooltipData.code || tooltipData.countryName}
                      data={tooltipData}
                    />
                  );
                }}
              ...
              {!isMarkerMapMode(mapMode) && (
                <div
                  ...
                  aria-label={__("Map legend", "bimbeau-privacy-analytics")}
                >
